...
class ContainerLinkSharePasswordViolated extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            filterByPassword_Data: null,
            filterByPassword_Error: null,
            itemSelected: null,
            subscription: null,
            modal: false,
            hours: null,
    ...
    updateData() {
        dashboardService.filterByPassword()
            .then(
                filterByPassword_Data => {
                    this.setState({ filterByPassword_Data });
                },
                filterByPassword_Error => {
                    this.setState({ filterByPassword_Error });
                }
            );
    }
    ...
    render() {
        const {filterByPassword_Data, filterByPassword_Error, modal, fade, viewType, data } = this.state;
        return (
            <div>
                {filterByPassword_Data &&
                    <div>
                        ...
                        <div>
                            {viewType  === 'ListView' &&
                                <List_Admin data={filterByPassword_Data}></List_Admin>
                            }
                            {viewType  === 'DonutView' &&
                                <Donut_Admin data={filterByPassword_Data}></Donut_Admin>
                            }
                        </div>
                    </div>
                }
                {modal === true &&
                    <div >
                        <div className="content-section implementation">
                            ...
                            modal={modal} fade={fade} toggle={this.toggleDialog_ForReports} type='ByDoc' documents={filterByPassword_Data}  hours="24"/>
                        </div>
                    </div>
                }
                {filterByPassword_Error &&
...
export default  ContainerLinkSharePasswordViolated;